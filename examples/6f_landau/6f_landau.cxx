...
const char CXXVERSION[] = "v0.2.1_02042018_J.G.Chen";
...
BoutReal q95_input;
bool local_q;
bool n0_fake_prof, n0_p0_0p3, T0_fake_prof, impurity_prof, Nimp_lowlimit, quasi_neutral_Ni, load_impurity, impurity_gyro;
...
int physics_init(bool restarting) {
  ...
  if (spitzer_resist) {
    ...
    eta = 0.51 * 1.03e-4 * Zi * LnLambda * ((Te0 * Tebar) ^ (-1.5)); // eta in Ohm-m. NOTE: ln(Lambda) = 20
...
int physics_run(BoutReal t) {
  ...
  if (nonlinear) {
    ...
    if (spitzer_resist) {
      // Use Spitzer formula
      eta = 0.51 * 1.03e-4 * Zi * LnLambda * ((Te_tmp * Tebar) ^ (-1.5)); // eta in Ohm-m. ln(Lambda) = 20
      eta /= MU0 * Va * Lbar;
      // eta.applyBoundary();
      // mesh->communicate(eta);
    } else {
  ...
  if (diffusion_par && Landau) {
    if (Landau_coll) {
      q_par_i = -Landau_coeff * (2. / 3.) * N0 * 2. * sqrt(2. / PI) * vth_i / Va * iSign_kpar_wcoll(Ti, kappa_i, nLorentzian);
      q_par_e = -Landau_coeff * (2. / 3.) * Ne0 * 2. * sqrt(2. / PI) * vth_e / Va * iSign_kpar_wcoll(Te, kappa_e, nLorentzian);
    } else {
      q_par_i = -Landau_coeff * (2. / 3.) * N0 * 2. * sqrt(2. / PI) * vth_i / Va * iSign_kpar(Ti, kappa_0, nLorentzian);
      q_par_e = -Landau_coeff * (2. / 3.) * Ne0 * 2. * sqrt(2. / PI) * vth_e / Va * iSign_kpar(Te, kappa_0, nLorentzian);
    }
