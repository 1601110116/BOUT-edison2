...
BoutReal Mp = 1.6726e-27;       // [kg] proton mass
BoutReal AA;                    // ion mass in units of the proton mass; AA=Mi/Mp
BoutReal Mi;                    // [kg] Mi = Mp*AA;
BoutReal ratio_pe = 1836.2;     // proton/electron mass ratio
...
const Field3D lowPass_pos2(const Field3D &var, const Field3D &prof)
{
  ...
  tmp_result = lowPass(var, low_pass_z, 0);
        ...
        for (int jy=0;jy<mesh->ngy;jy++)
	{
	  ...
	  if (keep_zonalPF && ( ((y_ind>int(jysep1)) && (y_ind<=int(jysep1_2))) || ((y_ind>int(jysep2_1)) && (y_ind<=int(jysep2))) ) )
          {
            if ( prof2d[jx][jy] < 0. )
	      {
  ...
  for(jx=0;jx<mesh->ngx;jx++)
    {
	  ...
	  if ( (dindx < PF_limit_range) && ((indy <= int(jysep1)) || ((indy > int(jysep1_2)) && (indy <= int(jysep2_1))) || (indy > int(jysep2))) )
	    {
	      for(jz=0; jz<mesh->ngz; jz++)
		result[jx][jy][jz] = 0.;
	    }
	  ...
	  for (jy=0;jy<mesh->ngy;jy++)
	    {
	      ...
	      if ( mgx > xgrid_num || (globaly<=int(jysep1)) || ((globaly>int(jysep1_2)) && (globaly<=int(jysep2_1))) || (globaly>int(jysep2)) )
		mgx = xgrid_num;
  ...
  if(diamag && diamag_phi0)
	  ...
	  if (diamag_er)
	    {
	      ...
	      Ve0 = Er0^B0vec;
	      Ve0.toCovariant();
	      dump.add(Er0.x, "Er0x",0);
	      dump.add(Er0.y, "Er0y",0);
	      dump.add(Er0.z, "Er0z",0);
	      ...
	      dump.add(Er0_balance.x, "Er0bx",0);
	      dump.add(Er0_balance.y, "Er0by",0);
	      dump.add(Er0_balance.z, "Er0bz",0);
      ...
      else
	{
	  if (diamag_er)
		...
		Er0y = Er0.y;
		Er0z = Er0.z;
		SAVE_ONCE3(Er0x,Er0y,Er0z);*/


	      test_phi0.setLocation(CELL_CENTRE);
	      test_er0.setLocation(CELL_CENTRE);
	      dump.add(test_phi0, "test_phi0", 0);
	      dump.add(test_er0, "test_er0", 0);
...
//  output.write("I see you 3! \n");//xia
  ...
  if (!emass)
	...
	ddt(Psi) = -Grad_parP(phi, CELL_CENTRE) - eta*Jpar;
      ...
      /*
      if(experiment_Er){
        ddt(Psi) -= V_dot_Grad(Ve0net, Psi)/B0;
      }
      */
...
*/
  ...
  if (output_vradial)
    {
      if (diamag_er)
        Vexb = (Ve0+(B0vec^Grad(phi*B0)))/(B0*B0);
      else
        Vexb = (B0vec^Grad((phi+phi0)*B0))/(B0*B0);
      mesh->communicate(Vexb);
      Vexb.applyBoundary();
      Vbtilde = -(B0vec^Grad(Psi))/B0;
