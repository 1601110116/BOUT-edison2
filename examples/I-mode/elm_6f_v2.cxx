...
/**** Declaration for temparary variables ************************/
...
/**** Halo current **********************************************/
bool halo_current;
BoutReal J_halo_width, J_halo_length, J_halo;
Field3D psi_h, Jpar_h;
int psih_flag;
Field3D Bh_r, Bh_y, Bh_z;
/****************************************************************/
...
bool local_q;
BoutReal q_alpha;
bool n0_fake_prof, T0_fake_prof, impurity_prof, Nimp_lowlimit, quasi_neutral_Ni, load_impurity, impurity_gyro;
BoutReal Zi, Z_imp, A_imp; // charge number of ion
...
// V0 field vectors
Vector2D Ve0net,Ve0; //net flow 
Vector2D Er0, Er0_balance;   //using Er0
...
BoutReal Mp = 1.6726e-27; // proton mass
BoutReal Mi;
BoutReal ratio_pe = 1836.2; 
...
const Field3D prof_Jhalo(const BoutReal &width, const BoutReal &length);//halo_current of helical current


const Field3D prof_Jhalo(const BoutReal &width, const BoutReal &length)
{
  Field3D result;
  result.allocate();
  ...
  BoutReal Grid_NX, Grid_NXlimit, Grid_sep, sep_length1, sep_length2;    //the grid number on x, and the
  mesh->get(Grid_NX, "nx");
//  int Grid_sep, sep_length1, sep_length2;

  Grid_sep = Grid_NX - ixsep;
  sep_length1 = ixsep + Grid_sep*length;
  sep_length2 = Grid_sep*width/2.;

  if (sep_length1 >= Grid_NX)
    sep_length1 = Grid_NX-1;
//  if (sep_length2 >= Grid_NX)
//    sep_length2 = Grid_NX-1;


//  output.write("sep1: %i.\n",int(sep_length1));
//  output.write("sep2: %i.\n",int(sep_length2));

  BoutReal f0, f1, f2, f3, mgz;
  f0 = 0.2;
  f1 = 1.;
  f2 = PI;
  f3 = 1.;
  ...
  for(jz=0;jz<mesh->ngz;jz++)
    {
      mgz = BoutReal(jz)/BoutReal(mesh->ngz)*2.*PI;
//      output.write("mgz = %f\n",mgz);
      for(jx=0;jx<mesh->ngx;jx++)
        {
          BoutReal mgx = mesh->XGLOBAL(jx);
          BoutReal rlx = mgx - sep_length1;
          BoutReal temp = exp(-rlx*rlx/(2.*sep_length2*sep_length2));
	  temp -= exp(-sep_length1*sep_length1/(2.*sep_length2*sep_length2));
...
//              int globaly = mesh->YGLOBAL(jy);
              ...
              result[jx][jy][jz] = temp;
              result[jx][jy][jz] *= abs(B0[jx][jy])/Bbar;
	      result[jx][jy][jz] *= f0+f1* (cos((mgz-f2)*f3/2.))*(cos((mgz-f2)*f3/2.));
            }
        }
     }
...
//  output.write("Y index: %i, %i, %i, %i", int(jysep1), int(jysep1_2), int(jysep2_1), int(jysep2)); 
...
//            output.write("%i, %i, %i, %i",jysep1, jysep1_2, jysep2_1, jysep2);
            }
          else
            result[jx][jy] = 0.;
...
{ 
  ...
  for(jx=0;jx<mesh->ngx;jx++) 
	...
	{
	  indy = mesh->YGLOBAL(jy);
//	  if ( ((indy > int(jysep1)) && (indy <= int(jysep1_2))) || ((indy > int(jysep2_1)) && (indy <= int(jysep2))) )
	    {
...
//  output.write("%i, %i, %i, %i,\n",jysep1, jysep1_2, jysep2_1, jysep2);
  ...
  OPTION(options, halo_current,      false);   // The switch of the halo current module
  OPTION(options, J_halo_width,      0.2);     // The width of the guassian function for the halo current on x direction, relative to the SOL
  OPTION(options, J_halo_length,     0.2);    // The distance of the peak of the halo current from the separatrix, relative to the SOL
  OPTION(options, J_halo,            1);      // The amplitude of the peak of the halo current unit A/m^-2. The sigh means the current direction
  OPTION(options, psih_flag,         769);    //The boundary for magnetic flux by halo current
...
//      Dri_neo.setBoundary("kappa");
  ...
  if (output_vradial)
	...
	//Vbte_par.setBoundary("Vipar");
      ...
      dump.add(Vexb.x, "vexb_y", 1);
      dump.add(Vexb.x, "vexb_z", 1);
...
//      Dri_neo = (1.+1.6*q95_input*q95_input)*(1.+Tau_ie)*nu_i*rho_i*rho_i;
      xii_neo = (q95_input*q95_input)/epsilon/sqrt(epsilon)*nu_i*(rho_i^2.); 
  ...
  if (neoclassic_e)
    {
      ...
      xie_neo = (q95_input*q95_input)/epsilon/sqrt(epsilon)*nu_e*(rho_e^2.);
...
//      xie_neo.applyBoundary();
  ...
  if (halo_current)
    {
      Jpar_h = J_halo * prof_Jhalo(J_halo_width, J_halo_length);
      Jpar_h /= B0*Bbar/MU0/Lbar;
 ...
 *                        psi_h = psiLap->solve(Jpar_h, psi_h);
 *                        */

      psi_h = invert_laplace(Jpar_h, psih_flag, NULL);
      mesh->communicate(psi_h);
...
//      bh.covariant = false;
      ...
      bh.toContravariant();
      Bh_r = bh.x;
      mesh->communicate(Bh_r);
      Bh_r.applyBoundary();
      Bh_y = bh.y;
      mesh->communicate(Bh_y);
      Bh_y.applyBoundary();
      Bh_z = bh.z;
      mesh->communicate(Bh_z);
      Bh_z.applyBoundary();


      dump.add(Jpar_h, "Jpar_h", 0);
      dump.add(psi_h, "psi_h", 0);
      dump.add(Bh_r, "Bh_r", 0);
      dump.add(Bh_y, "Bh_y", 0);
      dump.add(Bh_z, "Bh_z", 0);
    }
	  ...
	  if (diamag_er)
	    {
	      ...
	      phi0 = -Upara0*Pi0/N0;
//	      Er0.covariant = false;
	      Er0.x = Er_tmp / ( Bbar*Va*Rxy*Bpxy );
	      Er0.y = 0.;
	      Er0.z = 0.;
	      /*          Field2D Er0x, Er0y, Er0z;
			  Er0x = Er0.x;
			  Er0y = Er0.y;
			  Er0z = Er0.z;*/
//	      Er0.toCovariant();
	      ...
	      Er0_balance = Upara0*Grad(Pi0)/N0;
	      mesh->communicate(Er0_balance.x);
	      mesh->communicate(Er0_balance.y);
	      mesh->communicate(Er0_balance.z);
	      (Er0_balance.x).applyBoundary();
	      (Er0_balance.y).applyBoundary();
	      (Er0_balance.z).applyBoundary();
	      Er0_balance.toCovariant();
	      Ve0net = (Er0-Er0_balance)^B0vec;
	      Ve0net.toCovariant(); 
	      /*          Field2D Er0x_b, Er0y_b, Er0z_b;
			  Er0x_b = Er0_balance.x;
			  Er0y_b = Er0_balance.y;
			  Er0z_b = Er0_balance.z;
			  SAVE_ONCE3(Er0x_b,Er0y_b,Er0z_b);*/
	      U0_net = B0vec*Curl(N0*Ve0net)/B0;
	      phi0 = 0.;
	  ...
	  if (diamag_er)
	    {
	      ...
	      phi0 = -Upara0*Pi0/N0;
//	      Er0.covariant = false;
	      Er0 = Upara0*Grad(Pi0)/N0;
	      mesh->communicate(Er0.x);
	      mesh->communicate(Er0.y);
	      mesh->communicate(Er0.z);
	      (Er0.x).applyBoundary();
	      (Er0.y).applyBoundary();
	      (Er0.z).applyBoundary();
//	      Er0.toCovariant();
  ...
  if (diamag && diamag_phi0) 
    {
      SAVE_ONCE(phi0);
      if (experiment_Er)
        SAVE_ONCE4(Er0, Er0_balance, Ve0, U0_net);
    }
...
//      phi_sh0 *= log( 2.*sqrt(PI)*(c_se0-J0*B0*Bbar/(MU0*Lbar)/(Ne0*Nbar*density*ee))/vth_e0 );
  ...
  if (BScurrent)
	...
	q95r = q95;
      ...
      nu_estar = 100.*nu_e * q95r*Lbar / (vth_e) / pow(Aratio,1.5);
      nu_istar = 100.*nu_i * q95r*Lbar / (vth_i) / pow(Aratio,1.5);
  ...
  if (fakerun)
    {
      ...
      dump.add(Btilde.x, "btilde_x", 1);
      dump.add(Btilde.y, "btilde_y", 1);
      dump.add(Btilde.z, "btilde_z", 1);
    }
...
const Field3D Grad_parP(const Field3D &f, CELL_LOC loc = CELL_DEFAULT)
{
  ...
  if(parallel_lagrange || parallel_project) {
    ...
    if(nonlinear) {
      ...
      if (halo_current)
	result -= bracket(psi_h, f, bm_mag)*B0; 
    }
  }
  ...
  if (nonlinear)
    {
      ...
      if (impurity_prof)
	Ne_tmp = field_larger(Ne0+Ni*Zi, N_tmp1);
      else
	Ne_tmp = N_tmp*Zi;
    }
      ...
      if (neoclassic_e)
        {
          ...
          xie_neo = (q95_input*q95_input)/epsilon/sqrt(epsilon)*nu_e*rho_e*rho_e;
...
//  output.write("I see you 2! \n");//xia
  ...
  if ( (gamma_i_BC > 0.0) && (gamma_e_BC > 0.0) )
      ...
      else
	{
	  ...
	  if(halo_current)
	    phi_sh -= Psipara1 * bracket(psi_h, Pe0, bm_mag) / Ne0;
        }
      if(thermal_force)
        {
          ...
          if(halo_current)
	    phi_sh -= 0.71 * Psipara1 * bracket(psi_h, Te0, bm_mag);
        }
...
/*	  phi_sh = -Te_tmp*Tebar;
  ...
  if (compress0)
	...
	if (impurity_prof)
	  {
	    Vepar = Vipar*N_tmp*Zi/Ne_tmp - B0 * (Jpar) / Ne_tmp * Vepara;
	    if (halo_current)
	      Vepar -= B0 * (Jpar_h) / Ne_tmp * Vepara;
	  }
	else
	  {
	    Vepar = Vipar - B0 * (Jpar) / N_tmp * Vepara;
	    if (halo_current)
	      Vepar -= B0 * (Jpar_h) / N_tmp * Vepara;
	  }
      else
	if (impurity_prof)
	  {
	    Vepar = Vipar*N0*Zi/Ne0 - B0 * (Jpar) / Ne0 * Vepara;
	    if(halo_current)
	      Vepar = Vipar*N0*Zi/Ne0 - B0 * (Jpar_h) / Ne0 * Vepara;
	  }
	else
	  {
	    Vepar = Vipar - B0 * (Jpar) / N0 * Vepara;
	    if(halo_current)
	      Vepar = Vipar - B0 * (Jpar_h) / N0 * Vepara;
	  }
      Vepar.applyBoundary();
  ...
  if (!emass)
      ...
      else    
	{
      ...
      if(diamag && diamag_phi0){
        if (diamag_er)
	  {
            ddt(Psi) -= V_dot_Grad(Ve0, Psi)/B0;
	    if(halo_current)
	      ddt(Psi) -= V_dot_Grad(Ve0, psi_h)/B0;
	  }
        else
	  {
 	    ddt(Psi) -= bracket(phi0, Psi, bm_exb)*B0;   // Equilibrium flow
	    if(halo_current)
	      ddt(Psi) -= bracket(phi0, psi_h, bm_exb)*B0;
	  }
      }
      ...
      if(thermal_force) 
	{
	  ...
	  if(halo_current)
	    ddt(Psi) -= 0.71 * Psipara1 * bracket(psi_h, Te0, bm_mag);
	}
      ...
      if(eHall) 
	{
	  ...
	  ddt(Psi) -=  Psipara1 * bracket(Psi, Pe0, bm_mag) / Ne0;
          if(halo_current)
	    ddt(Psi) -=  Psipara1 * bracket(psi_h, Pe0, bm_mag) / Ne0;
	}
      ...
      if (spitzer_resist)
	{
	  ...
	  if(halo_current)
	    ddt(Ajpar) -= eta_spitzer*Jpar_h;
	}
      else    
	{
	  ddt(Ajpar) = -Grad_parP(B0*phi, CELL_CENTRE) / B0 - eta*Jpar;
	  if(halo_current)
	    ddt(Ajpar) -= eta*Jpar_h;
	}
      
      if(diamag && diamag_phi0){
        if (diamag_er)
          ddt(Ajpar) -= V_dot_Grad(Ve0, Psi)/B0;
        else
...
//  output.write("I see you 4! \n");//xia
  ...
  if (BScurrent)
    {
      ...
      if(halo_current)
	ddt(U) = -(B0^2) * bracket(psi_h, Jpar_BS0, bm_mag)*B0;
    }
  else
    {
      ddt(U) = -(B0^2) * bracket(Psi, J0, bm_mag)*B0; // Grad j term
      if(halo_current)
        ddt(U) = -(B0^2) * bracket(psi_h, J0, bm_mag)*B0; // Grad j term
    }
  ...
  if(halo_current)
    ddt(U) += (B0^2)*Grad_parP(Jpar_h, CELL_CENTRE);

  if(diamag && diamag_phi0){
    if (diamag_er)
      ddt(U) -= V_dot_Grad(Ve0, U)/B0;
    else
  ...
  if(gyroviscous)
    {
      ...
      if (diamag_er && diamag_phi0)
        Dperp2Phi0 = -Div( B0vec^Ve0 )/B0 - Div(B0vec)*(B0vec*Er0*B0);
      else
      ...
      /*if (diamag_er){
      test_phi0 = Field3D(Delp2(phi0)*B0);
      test_er0 = -Div( B0vec^Ve0 )/B0 + Div(B0vec)*(B0vec*Er0*B0);
      //test_phi0 = bracket(phi0, U, bm_exb)*B0;
      //test_er0 = V_dot_Grad(Ve0, U)/B0;
      //test_phi0 = Field3D( Grad(phi0)*Grad(phi0) );
      //test_er0 = Er0*Er0;
      //test_er0 = -Er0*Grad(phi);
      //test_phi0 = Grad(phi0)*Grad(phi);

      mesh->communicate(test_er0);
      mesh->communicate(test_phi0);}*/
      ...
      if (diamag_er && diamag_phi0)
        ...
        GradPhi02 = Field3D( Grad(phi0)*Grad(phi0) );
      GradPhi02.applyBoundary();
      mesh->communicate(GradPhi02);
      ...
      if (diamag_er && diamag_phi0)
        bracketPhi0P = V_dot_Grad(Ve0, Pi)/B0;
      else
      ...
      if (diamag_er && diamag_phi0)
        ddt(U) += 0.5*Upara2*V_dot_Grad(Ve0, Dperp2Pi)/B0/B0;
      else
      ...
      if (impurity_prof && impurity_gyro)
	{
	  ...
	  bracketPhiPimp0 = bracket(phi, P_imp0, bm_exb)*B0;
	  bracketPhiPimp0.applyBoundary();
	  mesh->communicate(bracketPhiPimp0);

	  ddt(U) -= 0.5*Upara_imp*Upara2*bracket(P_imp0, Dperp2Phi, bm_exb);
	  ddt(U) += Upara_imp*Upara3*B0*bracket(N_imp0, GradcPhi, bm_exb);
	  ddt(U) += 0.5*Upara_imp*Upara2*bracket(phi, Dperp2Pimp0, bm_exb);
	  ddt(U) -= 0.5*Upara_imp*Upara2*Delp2(bracketPhiPimp0);
	}
...
*/
  ...
  if (continuity)
    {
      ...
      if (diamag_er && experiment_Er)
      ddt(Ti) -= 4.0/3.0 * Ti * b0xcv * (Ve0net ^ B0vec); // net flow
      if (diamag)
	  ...
	  if (nonlinear)
	    {
	      ...
	      if(halo_current)
		bracket1i -= bracket(psi_h, Ti+Ti0, bm_mag)*B0;
	    }
	  else
	    {
	      bracket1i = -bracket(Psi, Ti0, bm_mag)*B0;
	      if(halo_current)
		bracket1i -= bracket(psi_h, Ti0, bm_mag)*B0;
	    }
	  ...
	  if (nonlinear)
	    {
	      ...
	      if(halo_current)
		{
                  ddt(Ti) -= bracket(psi_h, kappa_par_i, bm_mag)*B0 * gradpar_ti/N0;
                  ddt(Ti) -= kappa_par_i *  bracket(psi_h, gradpar_ti, bm_mag)*B0/N0;
                  ddt(Ti) -= bracket(psi_h, kappa_par_i, bm_mag)*B0 * bracket1i/N0;
                  ddt(Ti) -= kappa_par_i *  bracket(psi_h, bracket1i, bm_mag)*B0/N0;
		}
	    }
  ...
  if (energy_flux)
      ...
      if (thermal_force)
	{
	  ...
	  if (halo_current)
	    ddt(Te) += 0.71 * 2.0/3.0 * Tepara3 * Te0 * B0 / Ne0 * Grad_parP(Jpar_h, CELL_CENTRE);
	  if(BScurrent)
	    {
	      ...
	      if(halo_current)
		{
                  ddt(Te) -= 0.71 * 2.0/3.0 * Tepara3 * Te0 * B0 / Ne0 * bracket(psi_h, Jpar_BS0, bm_mag) * B0;
		}	
	    }
	  else
	    {
	      ...
	      if(halo_current)
                {
		  ddt(Te) -= 0.71 * 2.0/3.0 * Tepara3 * Te0 * B0 / Ne0 * bracket(psi_h, J0, bm_mag) * B0;
		}
	    }
	}
      if (nonlinear)
	  ...
	  if (thermal_force)
	    {
	      ...
	      if(halo_current)
		ddt(Te) += 0.71 * 2.0/3.0 * Tepara3 * Te * B0 / Ne0 * Grad_par(Jpar_h, CELL_CENTRE);
	    }
	}
    }
      ...
      if (spitzer_resist)
	{
	  if (BScurrent)
	  ...
	  else
	    {
	      ...
	      if(halo_current)
		ddt(Te) += 4.0/3.0 * Tepara4 * eta_spitzer * B0*B0 * J0 * Jpar_h / Ne0;
	    }
	}
      else
	{
	  if (BScurrent)
	  ...
	  else
	    {
	      ...
	      if(halo_current)
		ddt(Te) += 4.0/3.0 * Tepara4 * eta * B0*B0 * J0 * Jpar_h / Ne0;
	    }
	}
      if (nonlinear)
	{
	  if (spitzer_resist)
	    {
	      ...
	      if(halo_current)
		{
		  ddt(Te) += 4.0/3.0 * Tepara4 * eta_spitzer * B0*B0 * Jpar_h * Jpar / Ne0;
		  ddt(Te) += 2.0/3.0 * Tepara4 * eta_spitzer * B0*B0 * Jpar_h * Jpar_h / Ne0;
		}
	    }
	  else
	    {
	      ...
	      if(halo_current)
		{
		  ddt(Te) += 4.0/3.0 * Tepara4 * eta * B0*B0 * Jpar_h * Jpar / Ne0;
		  ddt(Te) += 2.0/3.0 * Tepara4 * eta * B0*B0 * Jpar_h * Jpar_h / Ne0;
		}
	    }
	}
    }
	  ...
	  if (nonlinear)
	    {
	      ...
	      if(halo_current)
		bracket1e -= bracket(psi_h, Te+Te0, bm_mag)*B0;
	    }
	  else
	    {
	      bracket1e = -bracket(Psi, Te0, bm_mag)*B0;
	      if(halo_current)
		bracket1e -= bracket(psi_h, Te0, bm_mag)*B0;
	    }
	  ...
	  if (nonlinear)
	    {
	      ...
	      if(halo_current)
		{
                  ddt(Te) -= bracket(psi_h, kappa_par_e, bm_mag)*B0 * gradpar_te/Ne0;
                  ddt(Te) -= kappa_par_e *  bracket(psi_h, gradpar_te, bm_mag)*B0/Ne0;
                  ddt(Te) -= bracket(psi_h, kappa_par_e, bm_mag)*B0 * bracket1e/Ne0;
                  ddt(Te) -= kappa_par_e *  bracket(psi_h, bracket1e, bm_mag)*B0/Ne0;
		}
	    }
  ...
  if (compress0)   //parallel velocity equation
    {
      ...
      if (halo_current)
	{
	  ddt(Vipar) += Vipara * bracket(psi_h, P0, bm_mag) * B0 / N0;
	}

      if (diamag && diamag_phi0){
        if (diamag_er)
          ddt(Vipar) -= V_dot_Grad(Ve0, Vipar)/B0;
        else
      ...
      if (parallel_viscous && compress0)
	{
	  ...
	  ddt(Vipar) -= Vipara * (B0*sqrt(B0))  * Grad_parP(pi_ci/(B0*sqrt(B0)), CELL_YLOW) / N0; 
	}
