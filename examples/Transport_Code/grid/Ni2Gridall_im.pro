...
pro Ni2Gridall_im, filep0, filene, fileni, filete, fileti, fileer, filename, file_nimp;, smooth=smooth 
  ...
  if ( size(file_nimp,/type) ne 7 ) then begin
     print, "Wrong impurity density file."
     n_imp = fltarr(g.nx)
     ;return
  endif else begin
     restore,file_nimp
...
;  if keyword_set(impurity) then begin
;    restore, file_nimp  
;  endif
;  help
...
;hel, profni
  for i=xsep, nx-1 do begin
     for j=0, g.jyseps1_1 do begin
        ...
        profnimp[i,j] = profnimp[i,g.jyseps1_1+1]
     ...
     for j = g.jyseps1_2+1, g.ny_inner-1 do begin
        ...
        profnimp[i,j] = profnimp[i,g.jyseps1_2]
     ...
     for j = g.ny_inner, g.jyseps2_1 do begin
        ...
        profnimp[i,j] = profnimp[i,g.jyseps2_1+1]
     ...
     for j = g.jyseps2_2+1, ny-1 do begin
        ...
        profnimp[i,j] = profnimp[i,g.jyseps2_2]
...
;  f=file_export(filename, gnew)
  ...
  s = file_write(handle, 'N_imp', profnimp)
